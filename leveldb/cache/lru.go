...
func (r *lru) EvictAll() {
	r.mu.Lock()
	back1 := r.recent.prev
	for rn := back1; rn != &r.recent; rn = rn.prev {
		rn.n.CacheData = nil
	}
	back2 := r.frequent.prev
	for rn := back2; rn != &r.frequent; rn = rn.prev {
		rn.n.CacheData = nil
	}
	back3 := r.r1.prev
	for rn := back3; rn != &r.r1; rn = rn.prev {
		rn.n.CacheData = nil
	}
	back4 := r.f1.prev
	for rn := back4; rn != &r.f1; rn = rn.prev {
		rn.n.CacheData = nil
	}
	r.reset()
	r.mu.Unlock()
